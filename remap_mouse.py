#!/usr/bin/env python3
"""One movement report from the chosen mouse becomes at most one left click."""
...
def remap_events(events, output, left_held=False):
    report = []
    for event in events:
...
def main():
    ...
    with closing(InputDevice(path)) as mouse:
        ...
        with UInput(buttons_and_axes, name='mouse_move_click', phys='mouse_move_click') as output:
            ...
            remap_events(mouse.read_loop(), output, left_held)
